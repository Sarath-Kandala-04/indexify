...
export default function SubscriptionsPanel({ pendingAction, goTo }) {
  ...
  function openNewForm() { setEditingId(null); setForm(emptyForm()); setShowForm(true) }
  function openEditForm(s) {
    setEditingId(s.id)
    setForm({ name: s.name, amount: s.amount, billingCycle: s.billingCycle, nextBillingDate: s.nextBillingDate, category: s.category, status: s.status, notes: s.notes || '' })
    setShowForm(true)
  }
  ...
  function saveSubscription(event) {
    ...
    if (editingId) {
      ...
      const subscription = {
        ...
        nextBillingDate: form.nextBillingDate, category: form.category, status: form.status, notes: form.notes,
        createdAt: Date.now(), updatedAt: Date.now(), isPinned: false, links: [],
      }
  ...
  function saveLinks(id, selected) {
    setSubscriptions(subscriptions.map((s) => (s.id === id ? { ...s, links: selected } : s)))
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto w-full max-w-[1100px] px-8 pt-20 pb-5">
        <div className="flex items-start justify-between gap-6">
          <div>
            <h1 className="font-display text-3xl" style={{ color: 'var(--text)' }}>Subscriptions</h1>
            <p className="mt-1 text-sm" style={{ color: 'var(--text-dim)' }}>Keep track of your recurring expenses.</p>
          </div>
          <button onClick={openNewForm} className="flex shrink-0 items-center gap-2 rounded-md px-5 py-2 text-sm font-medium transition-colors" style={{ background: 'var(--accent)', color: '#0d1210' }}>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          <div className="rounded-lg px-4 py-3" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)' }}>
            <div className="text-[11px]" style={{ color: 'var(--text-dim)' }}>Active subscriptions</div>
            <div className="mt-1 text-xl font-semibold" style={{ color: 'var(--text)' }}>{activeSubscriptions.length}</div>
          </div>
          <div className="rounded-lg px-4 py-3" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)' }}>
            <div className="text-[11px]" style={{ color: 'var(--text-dim)' }}>Monthly cost</div>
            <div className="mt-1 text-xl font-semibold" style={{ color: 'var(--accent)' }}>{formatCurrency(monthlyTotal)}</div>
          </div>
          <div className="rounded-lg px-4 py-3" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)' }}>
            <div className="text-[11px]" style={{ color: 'var(--text-dim)' }}>Yearly cost</div>
            <div className="mt-1 text-xl font-semibold" style={{ color: 'var(--text)' }}>{formatCurrency(yearlyTotal)}</div>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          <div className="flex flex-1 items-center gap-2 rounded-md px-3 py-2" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)' }}>
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search subscriptions..." className="w-full bg-transparent text-sm outline-none" style={{ color: 'var(--text)' }} />
          </div>
          <select value={category} onChange={(event) => setCategory(event.target.value)} className="rounded-md px-3 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="shrink-0 text-right">
                          <div className="text-lg font-semibold" style={{ color: 'var(--text)' }}>{formatCurrency(subscription.amount)}</div>
                      ...
                      <div className="mt-4 flex items-center justify-between border-t pt-3" style={{ borderColor: 'var(--line)' }}>
                        ...
                        <div className="ml-3 flex shrink-0 items-center gap-1">
                          ...
                          <button onClick={() => deleteSubscription(subscription.id)} className="rounded-md p-2" title="Delete" style={{ color: 'var(--coral)' }}>
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0, 0, 0, 0.55)' }}>
          <div className="w-full max-w-lg rounded-xl p-6" style={{ background: 'var(--panel)', border: '1px solid var(--line)' }}>
            <div className="mb-5 flex items-center justify-between">
              <h2 className="font-display text-xl" style={{ color: 'var(--text)' }}>{editingId ? 'Edit subscription' : 'Add subscription'}</h2>
              <button onClick={closeForm} style={{ color: 'var(--text-dim)' }}><X size={18} /></button>
            </div>
            <form onSubmit={saveSubscription} className="flex flex-col gap-4">
              <div>
                <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Brand / Subscription name</label>
                <BrandPicker value={form.name} onChange={(val) => setForm((current) => ({ ...current, name: val }))} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Amount</label>
                  <input name="amount" type="number" min="0" step="0.01" value={form.amount} onChange={handleChange} placeholder="0.00" className="w-full rounded-md px-3 py-2 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }} required />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Billing cycle</label>
                  <select name="billingCycle" value={form.billingCycle} onChange={handleChange} className="w-full rounded-md px-3 py-2 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Next billing date</label>
                  <input name="nextBillingDate" type="date" value={form.nextBillingDate} onChange={handleChange} className="w-full rounded-md px-3 py-2 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }} required />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Category</label>
                  <select name="category" value={form.category} onChange={handleChange} className="w-full rounded-md px-3 py-2 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }}>
                    {CATEGORIES.map((item) => <option key={item} value={item}>{item}</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Status</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs" style={{ color: 'var(--text-dim)' }}>Notes</label>
                <textarea name="notes" value={form.notes} onChange={handleChange} placeholder="Optional notes..." rows={3} className="w-full resize-none rounded-md px-3 py-2 text-sm outline-none" style={{ background: 'var(--panel-2)', border: '1px solid var(--line)', color: 'var(--text)' }} />
              </div>
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={closeForm} className="rounded-md px-4 py-2 text-sm" style={{ color: 'var(--text-dim)', border: '1px solid var(--line)' }}>Cancel</button>
                <button type="submit" className="rounded-md px-4 py-2 text-sm font-medium" style={{ background: 'var(--accent)', color: '#0d1210' }}>{editingId ? 'Save changes' : 'Add subscription'}</button>
              </div>
            </form>
          </div>
        </div>
